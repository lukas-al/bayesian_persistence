...
def _(prepare_data, px, us_data):
    ...
    px.line(regression_df, y=['Y', 'avg_lag_inflation', 'unemployment_rate_lag1'], title="Input Data")
...
def _(df_pre_2000, results_summary, run_ols):
    prior_mean_coeffs, prior_std_errs, prior_vcov_coeffs, prior_sigma_sq_error, X_pre, Y_pre, prior_nobs, prior_results = run_ols(df_pre_2000)
...
def _(df_post_2000, results_summary, run_ols):
    ...
    ls_coeffs_post, ls_std_errs_post, _, sigma_sq_error_post, X_post, Y_post, post_nobs, _ = run_ols(df_post_2000)
