...
def _(px, us_data):
    px.line(
        ...
        labels={
            ...
            "core_cpi_12m_change": "12 Month Change in Core CPI",
        },
    ).show()
    return
...
def create_lag_features_MONTHLY(df_input, inflation_col_name):
    ...
    lag_cols_4_12 = [
        f"{inflation_col}_lag{i}" for i in range(4, 13)
    ]  # Lags 4 through 12
...
def _(np, pd, sm):
    def run_bayesian_estimation(
        df_post_1999_prepared, y_col, x_cols, gamma_tilde, V, weights
    ):
        """
        ...
        for w in weights:
            ...
            data_mean_component = (
                (1 - w) * (1 / sigma2_post) * XtX_post @ gamma_ls_post.to_numpy()
            )

            posterior_mean = posterior_variance @ (
                prior_mean_component + data_mean_component
            )
        ...
        results[0] = {"posterior_mean": gamma_ls_post, "posterior_se": model_post.bse}
...
@app.cell
def _(pm):
    def run_pymc_estimation(
        df_post_1999_prepared, y_col, x_cols, gamma_tilde, V_base, w
    ):
        """
        ...
        with pm.Model() as model_kiley:
            ...
            betas = pm.MvNormal(
                "betas", mu=gamma_tilde.values, cov=V_scaled, shape=len(x_cols)
            )
...
def _(np, stats):
    def plot_analytical_distribution(ax, posterior_mean, posterior_se, label_dist):
        ...
        """
        x_vals = np.linspace(
            posterior_mean - 4 * posterior_se, posterior_mean + 4 * posterior_se, 300
        )
        y_vals = stats.norm.pdf(x_vals, loc=posterior_mean, scale=posterior_se)
        ax.plot(
            x_vals, y_vals, color="red", linestyle="--", linewidth=2.5, label=label_dist
        )
...
def _(bayesian_results, pymc_results, x_cols_table2):
    param_to_plot_name = (
        "unemployment_rate_lag1"  # unemployment_rate_lag1 avg_1_12_lagged_inf
    )
    ...
    pymc_samples_for_param = pymc_trace.posterior["betas"].sel(
        betas_dim_0=param_to_plot_index
    )
    pymc_mean = pymc_samples_for_param.mean().item()
    pymc_se = (
        pymc_samples_for_param.std().item()
    )  # Standard deviation of the PyMC samples
    return (
        analytical_mean,
        analytical_se,
        param_to_plot_name,
...
def _(
    ...
    ax_dist.axvline(
        ...
        linestyle=":",
        linewidth=2,
        label=f"PyMC Mean: {pymc_mean:.3f}",
    )
    ax_dist.axvline(
        analytical_mean,
        color="red",
    ...
    ax_dist.text(
        analytical_mean,
        text_y_pos_dist * 0.95,
        f"{analytical_mean:.3f}",
        color="red",
        ha="center",
        va="bottom",
        fontsize=9,
    ...
    ax_dist.set_title(
        f"Posterior for '{param_to_plot_name}' (Prior Weight = {weight_to_plot})",
        fontsize=14,
    )
    ax_dist.set_ylabel("Density", fontsize=12)
    ax_dist.tick_params(
        axis="y", which="major", labelsize=10
    )  # Only y-axis for top plot
    ...
    ax_errorbar.errorbar(
        x=pymc_mean,
        y=errorbar_y_positions[0],
        xerr=pymc_se,
        fmt="o",
        color="C0",
        capsize=5,
        markersize=8,
        label=f"PyMC: {pymc_mean:.3f} ± {pymc_se:.3f}",
    )
    ax_errorbar.errorbar(
        x=analytical_mean,
        y=errorbar_y_positions[1],
        xerr=analytical_se,
        fmt="s",
        color="red",
        capsize=5,
        markersize=8,
        label=f"Analytical: {analytical_mean:.3f} ± {analytical_se:.3f}",
    )
    ...
    ax_errorbar.legend(
        fontsize=9, loc="upper center", bbox_to_anchor=(0.5, -0.25), ncol=2
...
def _(bayesian_results, np, pd, plt, pymc_results, x_cols_table2):
    def _():
        ...
        fig.suptitle(
            "Comparison of Posteriors Across All Weights and Parameters",
            fontsize=16,
            y=1.0,
        )
...
def _(bayesian_results, np, pd, plt, pymc_results, sns, x_cols_table2):
    def _():
        ...
        param_names = x_cols_table2
        weights = sorted(bayesian_results.keys())
        n_analytical_samples = (
            200000  # Number of samples to draw for the analytical normal distribution
        )
        ...
        for i, param in enumerate(param_names):
            for w in weights:
                ...
                pymc_trace = pymc_results[w]
                pymc_samples = (
                    pymc_trace.posterior["betas"].sel(betas_dim_0=i).values.flatten()
                )
                ...
                analytical_mean = analytical_res["posterior_mean"][param]
                analytical_se = analytical_res["posterior_se"][param]
                analytical_samples = np.random.normal(
                    analytical_mean, analytical_se, n_analytical_samples
                )
                all_samples_list.append(
                    pd.DataFrame(
                        {
...
def _(
    ...
    for w_val_2 in weights_to_test:
        ...
        fcst = iterative_forecast(
            coeffs_to_use, initial_inf_lags, unemployment_assumption
        )
...
def _(forecasts, go, us_data, y_col):
    ...
    plot_inflation_forecasts_plotly(
        us_data.loc["January 2019":"December 2021", y_col], forecasts
    )
    return
